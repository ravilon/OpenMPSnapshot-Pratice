...
ssize_t omp_scc_coloring(const graph *G, vert_t **scc_id, int num_threads) {
	...
	*scc_id = (vert_t *) malloc(G->n_verts * sizeof(vert_t));
	...
	for(uint8_t i = 0 ; i < 2 ; ++i) {
		...
		#pragma omp parallel for default (shared) num_threads (num_threads)  reduction (+:verts_removed) 
	...
	while(n_active_verts > 0) {
		...
		#pragma omp parallel for default (shared) num_threads(num_threads)  reduction (+:sccs_found, verts_removed)
