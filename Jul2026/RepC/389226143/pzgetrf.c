...
plasma_unused( amk );
#pragma omp task depend (in:amk[0])  depend (inout:a00[0])  priority(1)
{
...
minmtnt-k);
// panel
#pragma omp task depend(inout:a00[0:ma00k*na00k])  depend(inout:a20[0:lda20*nvak])  depend(out:ipiv[k*A.mb:mvak])  priority(1)
{
...
#pragma omp taskloop untied shared(barrier)  num_tasks(num_panel_threads)  priority(2)
...
#pragma omp task depend(in:a00[0:ma00k*na00k])  depend(in:a20[0:lda20*nvak])  depend(in:ipiv[k*A.mb:mvak])  depend(inout:a01[0:ldak*nvan])  depend(inout:a11[0:ma11k*na11n])  depend(inout:a21[0:lda21*nvan])  priority(n == k+1)
{
if (sequence->status == PlasmaSuccess) {
// geswp
int k1 = k*A.mb+1;
...
// insert dummy task
#pragma omp task depend (in:ipiv[m*A.mb])  depend (inout:ipiv[0])
{
int l = 1;
l++;
}
...
#pragma omp task depend(in:ipiv[0:imin(A.m,A.n)])  depend(inout:a10[0:ma10k*na00k])  depend(inout:a20[0:lda20*nvak])
{
if (sequence->status == PlasmaSuccess) {
plasma_desc_t view =
...
plasma_complex64_t *amk = A(m, k);
plasma_unused( amk );
#pragma omp task depend (in:a10[0])  depend (inout:amk[0])
{
// Do some funny work here. It appears so that the compiler
// might not insert the task if it is completely empty.
int l = 1;
