...
ipivk = &ipiv[k*A.mb];
a00 = A(k, k);
#pragma omp task depend(inout:a00[0:size_a00])  depend(out:ipivk[0:size_i])  priority(1)
{
...
a01 = A(k, n);
a11 = A(k+1, n);
#pragma omp task depend(in:a00[0:size_a00])  depend(inout:ipivk[0:size_i])  depend(inout:a01[0:size_a01])  depend(inout:a11[0:size_a11])  priority(n == k+1)
{
if (sequence->status == PlasmaSuccess) {
// geswp
int k1 = k*A.mb+1;
