...
#pragma omp task depend(in:a1[0:ma1*na])    depend(in:a2[0:ma2*na])    depend(in:b[0:A.mb*mvak])  depend(inout:c[0:mc*A.mb])
{
...
c1 = W5(m1*(A.mt-k-1));
c2 = W5(m2*(A.mt-k-1));
#pragma omp task depend(in:c2[0:mc*A.mb])  depend(inout:c1[0:mc*A.mb])
{
for (int m = k+1; m < A.mt; m++) {
int mvam = plasma_tile_mview(A, m);
#pragma omp task
...
c2_out = A(k+1, k);
#pragma omp task depend(in:c1[0:mc*A.mb])        depend(in:c2_in[0:mc2_in*nc2])  depend(in:c3_in[0:mc3_in*nc2])  depend(out:c2_out[0:mc2_out*nc2])
{
for (int m = k+1; m < A.mt; m++) {
int mvam = plasma_tile_mview(A, m);
int ldam = plasma_tile_mmain(A, m);
...
int mvan = plasma_tile_mview(A, n);
#pragma omp task depend(in:a1[0:ma1*na])      depend(in:a2[0:ma2*na])      depend(in:b[0:A.mb*mvak])    depend(in:c1_in[0:mc1_in*nc])  depend(in:c2_in[0:mc2_in*nc])  depend(out:c_out[0:mc_out*nc])
{
for (int m = k+1; m < A.mt; m++) {
int mvam = plasma_tile_mview(A, m);
int ldam = plasma_tile_mmain(A, m);
...
#pragma omp task depend(inout:a1[0:ma1*na])  depend(inout:a2[0:ma2*na])  depend(out:ipiv[k1-1:k2])
{
...
a1 = L(k+1, n);
a2 = L(A.mt-1, n);
#pragma omp task depend(in:ipiv[(k1-1):k2])  depend(inout:a1[0:ma1*na])  depend(inout:a2[0:ma2*na])
{
if (sequence->status == PlasmaSuccess) {
plasma_desc_t view =
...
#pragma omp task depend(in:ipiv[(k1-1):k2])  depend(inout:a1[0:mnt1])  depend(inout:a2[0:mnt2])
{
plasma_barrier_t barrier;
plasma_barrier_init(&barrier);
...
#pragma omp task depend(in:Tin10[0:A.mb*ldtk])      depend(in:Tin11[0:nvak*ldtk])      depend(in:Tin21[0:nvakp1*ldtkp1])  depend(out:Tout01[0:nvak*ldtkm1])  depend(out:Tout11[0:nvak*ldtk])    depend(out:Tout21[0:nvak*ldtkp1])  depend(out:Tout[0:nvak*ldtkp1])
{
plasma_core_zlacpy(
