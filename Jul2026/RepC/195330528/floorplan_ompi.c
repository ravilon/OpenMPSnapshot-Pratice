...
/* byvalue variables */
int _fip_id = id, id = _fip_id;
int ((* _fip_NWS)[ 64])[ 2] = &NWS, (NWS[ 64])[ 2];
int _fip_j = j, j = _fip_j;
int _fip_i = i, i = _fip_i;

/* byvalue variable initializations */
memcpy((void *) NWS, (void *) *_fip_NWS, sizeof(NWS));
...
/* (l530) #pragma omp task untied private(board, footprint, area) firstprivate(NWS, i, j, id, nn) shared(FOOTPRINT, BOARD, CELLS, MIN_AREA, MIN_FOOTPRINT, N, BEST_BOARD, nnc, bots_verbose_mode) */
{
struct __taskenv__ {
struct cell * (* CELLS);
char (* (* BOARD))[ 64];
...
_tenv->CELLS = &CELLS;
_tenv->BOARD = &BOARD;
_tenv->FOOTPRINT = &FOOTPRINT;
_tenv->nnc = &nnc;
/* byvalue variables */
_tenv->id = id;
memcpy((void *) _tenv->NWS, (void *) NWS, sizeof(NWS));
_tenv->j = j;
_tenv->i = i;
