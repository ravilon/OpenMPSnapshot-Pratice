  ...
  if (omp_in_final() || ort_task_throttling())
    {
      ...
      /* byvalue variables */
      long (* _fip_low1) = low1, (* low1) = _fip_low1;
      long (* _fip_split1) = split1, (* split1) = _fip_split1;
      long (* _fip_low2) = low2, (* low2) = _fip_low2;
      long (* _fip_split2) = split2, (* split2) = _fip_split2;
      long (* _fip_lowdest) = lowdest, (* lowdest) = _fip_lowdest;
  ...
  if (omp_in_final() || ort_task_throttling())
    {
      ...
      /* byvalue variables */
      long (* _fip_split1) = split1, (* split1) = _fip_split1;
      long (* _fip_high1) = high1, (* high1) = _fip_high1;
      long (* _fip_split2) = split2, (* split2) = _fip_split2;
      long (* _fip_high2) = high2, (* high2) = _fip_high2;
      long (* _fip_lowdest) = lowdest, (* lowdest) = _fip_lowdest;
      long int _fip_lowsize = lowsize, lowsize = _fip_lowsize;
  ...
  if (omp_in_final() || ort_task_throttling())
    {
      ...
      /* byvalue variables */
      long (* _fip_A) = A, (* A) = _fip_A;
      long (* _fip_tmpA) = tmpA, (* tmpA) = _fip_tmpA;
      long _fip_quarter = quarter, quarter = _fip_quarter;
  ...
  if (omp_in_final() || ort_task_throttling())
    {
      ...
      /* byvalue variables */
      long (* _fip_B) = B, (* B) = _fip_B;
      long (* _fip_tmpB) = tmpB, (* tmpB) = _fip_tmpB;
      long _fip_quarter = quarter, quarter = _fip_quarter;
  ...
  if (omp_in_final() || ort_task_throttling())
    {
      ...
      /* byvalue variables */
      long (* _fip_C) = C, (* C) = _fip_C;
      long (* _fip_tmpC) = tmpC, (* tmpC) = _fip_tmpC;
      long _fip_quarter = quarter, quarter = _fip_quarter;
  ...
  if (omp_in_final() || ort_task_throttling())
    {
      ...
      /* byvalue variables */
      long (* _fip_D) = D, (* D) = _fip_D;
      long (* _fip_tmpD) = tmpD, (* tmpD) = _fip_tmpD;
      long _fip_size = size, size = _fip_size;
      long _fip_quarter = quarter, quarter = _fip_quarter;
  ...
  if (omp_in_final() || ort_task_throttling())
    {
      ...
      long _fip_quarter = quarter, quarter = _fip_quarter;
      long (* _fip_D) = D, (* D) = _fip_D;
      long (* _fip_low) = low, (* low) = _fip_low;
      long _fip_size = size, size = _fip_size;
      long (* _fip_tmpC) = tmpC, (* tmpC) = _fip_tmpC;
  ...
  # 470 "sort.c"
  {
    ...
    if (ort_mysingle(1))
      if (omp_in_final() || ort_task_throttling())
        ...
        /* (l472) #pragma omp task untied */
        {
          ort_new_task(_taskFunc8_, (void *) 0, 0, 1);
        }
    ort_leaving_single();
  }
  CANCEL_parallel_470 :
