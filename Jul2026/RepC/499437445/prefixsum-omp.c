...
// Step 1 & 2: partition data among threads and calculate its prefix sum for each thread
...
// Step 3: use the previous algorithm to compute the prefix sum of the last elements from all threads
for (int t = 0; t < num_thread; t ++) {
int left = block_size * t + 1, right = MIN(block_size * (t + 1), n);
if (left <= right) prefix_sum[right] += prefix_sum[left-1];
}

// Step 4: use the prefix sum from the last elements to patch up the answers
#pragma omp parallel for
