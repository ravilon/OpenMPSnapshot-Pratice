...
typedef struct Net
{
	_Float64 weight0_1[INPUT][LAYER1][LENGTH_KERNEL][LENGTH_KERNEL];			//[1][6][5][5]
	_Float64 weight2_3[LAYER2][LAYER3][LENGTH_KERNEL][LENGTH_KERNEL];			//[6][16][5][5]
	_Float64 weight4_5[LAYER4][LAYER5][LENGTH_KERNEL][LENGTH_KERNEL];			//[16][120][5][5]
	_Float64 weight5_6[LAYER5 * LENGTH_FEATURE5 * LENGTH_FEATURE5][LAYER6];	//[120][256]
   _Float64 weight6_7[LAYER6][OUTPUT];	//[256][10]

	_Float64 bias0_1[LAYER1];//[6]
	_Float64 bias2_3[LAYER3];//[16]
	_Float64 bias4_5[LAYER5];//[120]
	_Float64 bias5_6[LAYER6];//[256]
	_Float64 bias6_7[OUTPUT];//[10]
}Net;

typedef struct Feature
{
	_Float64 input[INPUT][LENGTH_FEATURE0][LENGTH_FEATURE0];//32
	_Float64 layer1[LAYER1][LENGTH_FEATURE1][LENGTH_FEATURE1];//28
	_Float64 layer2[LAYER2][LENGTH_FEATURE2][LENGTH_FEATURE2];//14
	_Float64 layer3[LAYER3][LENGTH_FEATURE3][LENGTH_FEATURE3];//10
	_Float64 layer4[LAYER4][LENGTH_FEATURE4][LENGTH_FEATURE4];//5
	_Float64 layer5[LAYER5][LENGTH_FEATURE5][LENGTH_FEATURE5];//1
	_Float64 layer6[LAYER6];//256
	_Float64 output[OUTPUT];//10
}Feature;

/**_Float64 relu(_Float64 x)
{
...
void forward(Net *net, Feature *features)
{
	...
	// max pooling 
	//#pragma omp parallel for num_threads(4) //private(tempint)
	...
	//#pragma omp parallel for num_threads(4) private(tempBias, temp)
	for (int y = 0; y < 16; y++)
	{
