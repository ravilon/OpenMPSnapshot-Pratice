...
// FIXME: are we leaking a created openmpEvent here since the profiling
// function calls create? metaOpenMPDestroyEvent(frame->event.event_pl);
...
* \param face the structure defining the slab of the buffer that should be
* packed
* \param remain_dim a precomputed array of the combined sizes of all interior
* dimensions at all face levels
* \return the offset index in the full buffer for this idx to read/write
* to/from
*/
...
* \param packed_buf the output buffer, of sufficient size to store an entire
* face/slab
...
* \param packed_buf the input buffer, of sufficient size to store an entire
* face/slab
* \param buf the unpacked 3D buffer
...
* (i.e sum the cubic cell and its 6 directly-adjacent neighbors and divide by
* 7)
...
*   (i.e. will only compute values for cells in T([sx+1:ex-1], [sy+1:ey-1],
* [sz+1:ez-1])
