...
void gpu_initialize() {
	...
	istat = meta_set_acc(idevice, metaModePreferOpenCL);//TODO make "meta_set_acc"
#endif
...
#pragma omp master
	...
	//gy = 0
	//gz = 0
	// dimblock = {tx,ty,tz}; //TODO move into CUDA backend, replace with generic struct
	...
	//dimgrid = {gx,gy,1}; // TODO move into CUDA backend, replace with generic struct
	...
	case meta_in:
		printf("Test Dot-Product:\t%s\n\tExpect[%d] Returned[%d]\n",
				(*(int*) sum_dot_gpu
						...
						"PASSED" : "FAILED"),
				(ni - 2) * (nj - 2) * (nk - 2) * iters, (*(int*) sum_dot_gpu)); //print *, "Test Reduction:",sum_dot_gpu
