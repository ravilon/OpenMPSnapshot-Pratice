...
void solveModel (TwoBody *m)
{
    ...
    // Each thread will store its local forces in Phase 1
    vect_t *loc_forces = (vect_t*)malloc(sizeof(vect_t)*n*thread_count);

    // Time loop
    #pragma omp parallel num_threads(thread_count) default(none) \
    shared(p,forces,loc_forces,thread_count,dt,n,nsteps,output_freq) \
    private(step,part)
    for (step = 0; step < nsteps; step++)
    {
        ...
        // Parallel computing of position and velocity of each particle (no race condition here)
        #pragma omp for
        for (part = 0; part < n; part++)
