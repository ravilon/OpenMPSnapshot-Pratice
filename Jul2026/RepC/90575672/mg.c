...
int main()
{
  ...
  setup(&n1, &n2, &n3);
//---------------------------------------------------------------------
// Modified : Added #pragma omp parallel for zero3(u, n1, n2, n3);
//---------------------------------------------------------------------
#pragma omp parallel
{
  zero3(u, n1, n2, n3);
...
//---------------------------------------------------------------------
// Modified : Added #pragma omp parallel for norm2u3);
//---------------------------------------------------------------------
...
//---------------------------------------------------------------------
// Modified : Added #pragma omp single printing iterations,threads
//---------------------------------------------------------------------
#pragma omp single
{
  //  printf("\n");
  ...
  resid(u, v, r, n1, n2, n3, a, k);
//---------------------------------------------------------------------
// Modified : Added #pragma omp single for setup function);
//---------------------------------------------------------------------
  #pragma omp single
  setup(&n1, &n2, &n3);
  zero3(u, n1, n2, n3);
...
//---------------------------------------------------------------------
// Modified :Removed ragma omp parallel for default(shared) private(i1,i2,i3,r1,r2)
//---------------------------------------------------------------------
 // #pragma omp parallel for default(shared) private(i1,i2,i3,r1,r2)
//---------------------------------------------------------------------
// Modified : Added #pragma omp for psinv algorithm
//---------------------------------------------------------------------
#pragma omp for   
for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
  ...
  if (debug_vec[0] >= 1) {
//---------------------------------------------------------------------
// Modified : Added  #pragma omp single for rep_nrm
//---------------------------------------------------------------------
   #pragma omp single 
...
//---------------------------------------------------------------------
//Modified : Added  #pragma omp for calculation of array u.
//---------------------------------------------------------------------
#pragma omp for  
for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
...
//---------------------------------------------------------------------
 #pragma omp single
  ...
  if (debug_vec[2] >= k) {
//---------------------------------------------------------------------
//Modified : Added  #pragma omp single  for showall.
//---------------------------------------------------------------------
#pragma omp single    
showall(r, n1, n2, n3);
  }
                   ...
                   void *ou, int n1, int n2, int n3, int k)
{
  ...
  if (n1 != 3 && n2 != 3 && n3 != 3) {
//---------------------------------------------------------------------
//Modified : Removed #pragma omp parallel for default(shared) private(i1,i2,i3,z1,z2,z3)
//---------------------------------------------------------------------
   // #pragma omp parallel for default(shared) private(i1,i2,i3,z1,z2,z3)
//---------------------------------------------------------------------
//Modified : Added #pragma omp for for z1,z2,z3
//---------------------------------------------------------------------
#pragma omp for    
...
//---------------------------------------------------------------------
//Modified : removed  #pragma omp parallel default(shared) private(i1,i2,i3)
//---------------------------------------------------------------------
//    #pragma omp parallel default(shared) private(i1,i2,i3)
  //  {
//---------------------------------------------------------------------
//Modified : Added   #pragma omp for to caluculate u
//---------------------------------------------------------------------
    #pragma omp for
...
//---------------------------------------------------------------------
//Modified : Added   #pragma omp for to calculate u in all dimensions
//---------------------------------------------------------------------
...
//---------------------------------------------------------------------
//Modified : #pragma omp single for   rep_nrm(z, mm1, mm2, mm3, "z: inter", k-1); and rep_nrm(u, n1, n2, n3, "u: inter", k);
//---------------------------------------------------------------------
#pragma omp single
  {
  if (debug_vec[0] >= 1) {
                    ...
                    int nx, int ny, int nz)
{
  ...
  max_rnmu = 0.0;
//---------------------------------------------------------------------
//Modified : removed  ##pragma omp parallel default(shared) private(i1,i2,i3,a) reduction(+:s)
//---------------------------------------------------------------------
  //#pragma omp parallel default(shared) private(i1,i2,i3,a) reduction(+:s)
  //{
    double my_rnmu = 0.0;
//---------------------------------------------------------------------
//Modified : Added  #pragma omp for for my_rnmu.
//---------------------------------------------------------------------
    #pragma omp for //nowait
    for (i3 = 1; i3 < n3-1; i3++) {
      for (i2 = 1; i2 < n2-1; i2++) {
  ...
  *rnmu = max_rnmu;
//---------------------------------------------------------------------
//Modified : Added #pragma omp barrier
//---------------------------------------------------------------------
#pragma omp barrier
//---------------------------------------------------------------------
//Modified : Added  #pragma omp single  for rnm2 = sqrt(s / dn);
//---------------------------------------------------------------------
#pragma omp single
    {
  *rnm2 = sqrt(s / dn);
...
static void comm3(void *ou, int n1, int n2, int n3, int kk)
{
  ...
  //{
//---------------------------------------------------------------------
//Modified : Added  #pragma omp for to calculate u by axis x.
//---------------------------------------------------------------------
  #pragma omp for
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
...
//---------------------------------------------------------------------
//Modified : Added  #pragma omp for to calculate u by axis y.
//---------------------------------------------------------------------
  #pragma omp for
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i1 = 0; i1 < n1; i1++) {
...
//---------------------------------------------------------------------
//Modified : Added  #pragma omp for to calculate u by axis z.
//---------------------------------------------------------------------
  #pragma omp for //nowait
  for (i2 = 0; i2 < n2; i2++) {
    for (i1 = 0; i1 < n1; i1++) {
...
static void zran3(void *oz, int n1, int n2, int n3, int nx1, int ny1, int k)
{
  ...
  a2 = power(a, nx1*ny1);
//---------------------------------------------------------------------
//Modified : Added  #pragma omp parallel  for zero3
//---------------------------------------------------------------------
#pragma omp parallel
  {
  zero3(z, n1, n2, n3);
  ...
  //---------------------------------------------------------------------
//---------------------------------------------------------------------
//Modified : removed  #pragma omp parallel for default(shared) private(i2,i3,x1,xx,rdummy) 
//---------------------------------------------------------------------
 // #pragma omp parallel for default(shared) private(i2,i3,x1,xx,rdummy) \            shared(e2,e3,d1,a1)
  for (i3 = 1; i3 < e3; i3++) {
    x1 = starts[i3];
    for (i2 = 1; i2 < e2; i2++) {
      xx = x1;
  ...
  //---------------------------------------------------------------------
//---------------------------------------------------------------------
//Modified : removed  #pragma omp parallel default(shared) private(i,i0,i1,i2,i3,j1,j2,j3,ten\  myid,num_threads) shared(best0,best1,n1,n2,n3)
//---------------------------------------------------------------------
  //#pragma omp parallel default(shared) private(i,i0,i1,i2,i3,j1,j2,j3,ten, \  myid,num_threads) shared(best0,best1,n1,n2,n3)
  //{
  for (i = 0; i < mm; i++) {
    ten[i][1] = 0.0;
    j1[i][1] = 0;
...
//---------------------------------------------------------------------
//Modified : Added  #pragma omp foraray ten[][][].
//---------------------------------------------------------------------
  #pragma omp for
  for (i3 = 1; i3 < n3-1; i3++) {
    double (*zi3)[n1] = z[i3];
  ...
  for (i = mm - 1; i >= 0; i--) {
   ...
   // }
//---------------------------------------------------------------------
//Modified : removed #pragma omp for ordered schedule(static)
//---------------------------------------------------------------------
  //  #pragma omp for ordered schedule(static)
    for (i2 = 1; i2 <= num_threads; i2++) {
//---------------------------------------------------------------------
//Modified : Added  #pragma omp ordered to calculate jg.
//---------------------------------------------------------------------
     #pragma omp ordered
    {
        if (ten[i1][1] > best1) {
...
//---------------------------------------------------------------------
//Modified : Added  #pragma omp parallel for private(i2, i1) to intilize 0 for all array values
//---------------------------------------------------------------------
...
static void zero3(void *oz, int n1, int n2, int n3)
{
  ...
  int i1, i2, i3;
//---------------------------------------------------------------------
//Modified : Added  #pragma omp parallel for default(shared) private(i1,i2,i3) for z
//---------------------------------------------------------------------
  #pragma omp parallel for default(shared) private(i1,i2,i3)
  for (i3 = 0; i3 < n3; i3++) {
    for (i2 = 0; i2 < n2; i2++) {
