 ...
 *
 * Compile with
 * gcc -std=c99 -Wall -Wpedantic -fopenmp omp-mandelbrot.c gfx.c -o omp-mandelbrot.out -lX11
 *
 * Run with:
 * OMP_NUM_THREADS=4 ./omp-mandelbrot.out
 *
 * If you enable the "runtime" scheduling clause, you can select the
 * scheduling type at runtime, e.g.,
 *
 * OMP_NUM_THREADS=4 OMP_SCHEDULE="static,64" ./omp-mandelbrot.out
 *
