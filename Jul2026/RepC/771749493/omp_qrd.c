...
void omp_qrd(struct Matrices* m)
{
    ...
    for(k = 0; k < n; k++)
    {
        ...
        #pragma omp parallel for schedule(static) reduction(+: nrm)  default(none) shared(A) firstprivate(k, n) private(i)
        for(i = 0; i < n; i++)
            nrm += A[i * n + k] * A[i * n + k];
         ...
         */
        #pragma omp parallel for simd schedule(static)  default(none) shared(A, Q, R) firstprivate(k, n) private(i)
        for(i = 0; i < n; i++)
            Q[i * n + k] = A[i * n + k] / R[k * n + k];

        // Distributes the workload using dynamic load balancing.
        #pragma omp parallel for schedule(dynamic, 1)  default(none) shared(A, Q, R) firstprivate(k, n) private(j, i, red1)
        for(j = k + 1; j < n; j++)
        {
            red1 = 0;
...
void omp_target_qrd(struct Matrices* m)
{
    ...
    #pragma omp target teams  map(tofrom: A[0:n*n], Q[0:n*n], R[0:n*n])  map(to: n, i, j, k, nrm, red1)
    for (k = 0; k < n; k++)
    {
        nrm = 0;
