...
#include <x86intrin.h> //to compile with sse on stoker
...
void team_conv(float ***image, float ****kernels, float ***output,
    ...
    int m;
    //Iterate over each kernel in parallel
        ...
        for (w = 0; w < m_width; w++)
        {
            ...
            for (h = 0; h < m_height; h++)
            {
                ...
                for (c = 0; c < m_c; c++)
                {
                    ...
                    for (x = 0; x < m_ko; x++)
                    {
                        //access two values from image & kernels when loading from memory into cache
                        //this reduces time wasted loading from memory into cache
                        for (y = 0; y < m_ko - 1; y += 2)
                        {
                            sum += image[w + x][h + y][c] * kernels[m][c][x][y];
                            sum += image[w + x][h + y + 1][c] * kernels[m][c][x][y + 1];
                        }
                        //account for final (odd) case in sum value
                        sum += image[w + x][h + y][c] * kernels[m][c][x][y];
                    }
                }
                //output value assignment moved outside of loop to reduce time wasted repeatedly assigning value
                output[m][w][h] = sum;
            }
        }
    }
