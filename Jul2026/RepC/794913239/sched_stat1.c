...
void combSort(float arr[], int n) {
    ...
    while (gap > 1 || swapped) {
        ...
        #pragma omp parallel for default(none) shared(arr, gap, n, swapped) schedule(static, 1)
...
int main(int argc, char* argv[]) {
    ...
    for (i = 0; i < 100; i++) { // 100 экспериментов
        ...
        #pragma omp parallel for default(none) shared(M1, N) schedule(static, 1)
        for (int j = 0; j < N; j++) {
            M1[j] = tanh(M1[j]) - 1;
        }
        ...
        #pragma omp parallel for default(none) shared(M2, M2_copy, N) schedule(static, 1)
        for(int j = 0; j < N/2; j++){
            M2_copy[j] = M2[j];
        }
        ...
        #pragma omp parallel for default(none) shared(M2, M1, N) schedule(static, 1)
        for(int j = 0; j < N/2; j++){
            M2[j] = M1[j]/M2[j];
        } 
        ...
        float min = M2[0];
        float x = 0.0f;
        #pragma omp parallel for default(none) shared(M2, min, N) reduction(+:x) schedule(static, 1)
        for (int j = 1; j < N / 2; j++) {
            if ((int)(M2[j] / min) % 2 == 0) {
                x += sin(M2[j]);
            }
