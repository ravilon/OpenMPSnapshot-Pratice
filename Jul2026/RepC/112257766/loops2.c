...
void valid1(void);
void valid2(void);
/* Run specified loop with given chunk */
...
{
  work_queue *work_queues;      // work queue array for threads
  omp_lock_t *work_queue_locks; // locks for each work queue
...
#pragma omp barrier
    ...
    while (1)
    {
      ...
      if (-1 == idx)
      {
        /* No work left, terminate */
        break;
      }

      /* Work stealing from most loaded thread */
      chunk stolen_work;
      omp_set_lock(&work_queue_locks[idx]);
      if (work_queues[idx].workload != left_workload)
      {
        /* Work stealed by others first , skip */
...
/* 
 * run specified loop with given chunk
 * in : loopid, loop type identification
 * in : lo, lower bound of the loop
 * in : hi, higher bound of the loop
 */
