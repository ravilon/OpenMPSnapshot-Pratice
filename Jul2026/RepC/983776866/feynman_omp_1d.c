...
#pragma omp parallel default(none) shared(a, h, stepsz, N, n_inside, w_exact, wt, err)  shared(seed) 
{
for (int i = 1; i <= NI; i++)
{
...
#pragma omp parallel default(none) shared(a, h, stepsz, N, seed)  reduction(+ : err)  reduction(+ : n_inside)
{
