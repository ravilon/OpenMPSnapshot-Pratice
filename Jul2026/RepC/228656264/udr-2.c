...
#pragma omp declare reduction (foo : int : omp_out += omp_in) initializer (omp_priv = int ())
void baz (int v)
{
S s;
int q = 0;
if (s.s != 6 || v != 0) abort ();
s.s = 20;
#pragma omp parallel num_threads (4) reduction (foo : s, v) reduction (::NS::U::operator + : q)
{
...
#pragma omp declare reduction (bar : S : omp_out.foo (omp_in)) initializer (omp_priv (8))
}
