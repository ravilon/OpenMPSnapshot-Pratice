...
double best_nearby(double delta[MAXVARS], double point[MAXVARS], double prevbest, int nvars)
{
	...
	for (i = 0; i < nvars; i++)
		z[i] = point[i];
	/* cannot be parallelized cause f() is dependent to previous z values */
	for (i = 0; i < nvars; i++)
	{
		z[i] = point[i] + delta[i];
		ftmp = f(z, nvars);
...
int main(int argc, char *argv[])
{
	...
	/* Telling to the compiler that only the main
	 thread can make MPI calls */
	int rank, size, provider;
	MPI_Init_thread(&argc, &argv, MPI_THREAD_FUNNELED, &provider);

	if (provider < MPI_THREAD_FUNNELED){
	...
	int my_start = ntrials*rank / size;
	int my_end = ntrials*(rank+1) / size;
	int chunk_size = my_end - my_start;

	if(!rank)
		t0 = get_wtime();

	//double my_best_fx = 1e10;

	#pragma omp parallel for private(i, startpt, endpt, fx)
	for (trial = my_start; trial < my_end; trial++)
	{
		/* starting guess for rosenbrock test function, search space in [-4, 4) */
			//#pragma omp for
			//for (i = 0; i < MAXVARS; i++) my_best_pt[i] = 0.0;
	...
	if(!rank)
	{
		//MPI_Reduce(&my_best_fx, &best_fx, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
