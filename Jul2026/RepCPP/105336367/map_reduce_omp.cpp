...
private:
	...
	std::list<string>      stringEntries;	    // auxillary list for counting the of chars in the content
...
// add partialStringTotal pair values to class-member stringTotal pair values
		...
		#pragma omp single
		{
			while (start < contentSize) {
				...
				#pragma omp task firstprivate(start, end)
				{
					...
					Map(partialStringEntries, start, end);					// Collect the word entries
					Reduce(partialStringTotal, partialStringEntries);		// Count the number of entry of each word
