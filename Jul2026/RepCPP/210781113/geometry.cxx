                  ...
                  int_vec2D &support)
{
    ...
    #pragma omp parallel for default(none)       shared(var,tmp_result,mat,volume,pseudo_speed,is_quasi_static,  has_thermal_diffusion)
    #pragma acc parallel loop
    for (int e=0;e<var.nelem;e++) {
    ...
    #pragma omp parallel for default(none)       shared(var,volume_n,mass,tmass,tmp_result,support,has_thermal_diffusion)
    #pragma acc parallel loop
    for (int n=0;n<var.nnode;n++) {
        volume_n[n]=0;
        mass[n]=0;
                      ...
                      shapefn &shpdx, shapefn &shpdy, shapefn &shpdz)
{
    ...
    #pragma omp parallel for default(none)       shared(shpdx, shpdy, shpdz, connectivity, coord, volume, var_nelem)
    // #pragma acc parallel loop
