...
#pragma omp parallel for firstprivate(rho,rhou,rhov,E, rho_new, rhou_new, rhov_new, E_new)
{
...
/*Threads wait each other as if there was a barrier after each for loop. */
...
/*Comment of Student: This is the reason why parallelization on the main loop is not possible. 
Values are used in the next iteration.*/
#pragma omp parallel for
{
for (int i = 1; i <= Nx; i++){
for (int j = 1; j <= Ny; j++){
