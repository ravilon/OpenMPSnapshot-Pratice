...
return a.weight / (float) a.cost > b.weight / (float) a.cost;
}
...
=======
int rests[MAXIMAL];
int lightest[MAXIMAL];
int nextCost[MAXIMAL];

int cmp(const item& a, const item& b) {
return a.cost < b.cost;
}

#define PRICE_UNDEF -1
bool rekser(int depth, int freeCap, int curPrice)
{
int retA = false;
int retB = false;

if(depth == no_items) {
if(curPrice > tmp_max && freeCap >= 0)
return (tmp_max = curPrice);
return false;
}
...
if(curPrice + rests[depth] <= tmp_max)
return false;

if((freeCap - lightest[depth]) < 0 && curPrice <= tmp_max)
return false;

if((freeCap / lightest[depth]) * nextCost[depth] + curPrice <= tmp_max)
return false;

retB = rekser(depth + 1, freeCap, curPrice);
retA = rekser(depth + 1, freeCap - array_of_items[depth].weight, curPrice + array_of_items[depth].cost);

if(retA) max_state[depth] = 1;
else if(retB)
max_state[depth] = 0;
return retA || retB;
}

void rek(int depth, int freeCap, int curPrice) {

// qsort(array_of_items, no_items, sizeof(item), cmp);
std::sort(array_of_items, array_of_items + no_items - 1, cmp);
rests[no_items - 1] = array_of_items[no_items - 1].cost;
lightest[no_items - 1] = array_of_items[no_items - 1].weight;
nextCost[no_items - 1] = array_of_items[no_items - 1].cost;

for(int x = no_items - 2; x >= 0; x--) {
rests[x] = rests[x + 1] + array_of_items[x].cost;
lightest[x] = array_of_items[x].weight < lightest[x + 1] ? array_of_items[x].weight : lightest[x + 1];
nextCost[x] = array_of_items[x].cost > nextCost[x + 1] ? array_of_items[x].cost : nextCost[x + 1];
}
rekser(depth, freeCap, curPrice);
}
>>>>>>> 1fe7a87afc2fd1add6bee3b702d1bfc385a79421
