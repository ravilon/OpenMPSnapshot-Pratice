...
PGM_IMG contrast_enhancement_g(PGM_IMG_MPI img_in)
{
    ...
    int img_size_g = img_in.width * img_in.height;
    int remainder_g = img_size_g % size;

    if (rank == 0) {
        result.img = (unsigned char *)malloc(img_in.width * img_in.height * sizeof(unsigned char));
    ...
    histogram(local_hist, img_in.segment_in_g, img_in.segment_size_g, 256);
    ...
    unsigned char *enhanced_segment = (unsigned char *)malloc(img_in.segment_size_g * sizeof(unsigned char));
    histogram_equalization(enhanced_segment, img_in.segment_in_g, global_hist, img_in.segment_size_g, img_size_g, 256);
    ...
    MPI_Gatherv(enhanced_segment, img_in.segment_size_g, MPI_UNSIGNED_CHAR, result.img, sendcounts, displs, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
