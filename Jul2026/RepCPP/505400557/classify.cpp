...
{ // Classify each item in D into intervals (given by R). Finally, produce in D2 data sorted by interval
   ...
   #pragma omp parallel num_threads(numt)
   {
      ...
      for(int i=(D.ndata/numt)*tid; i<std::min(D.ndata,(tid+1)*(D.ndata)/numt); i+=1) { // Threads together share-loop through all of Data
   ...
   #pragma omp parallel num_threads(numt)
   {
      ...
      for(int r=(D.ndata/numt)*tid; r<R.num(); r+=1) { // Thread together share-loop through the intervals 
         int rcount = 0;
         int l =0;
         int h = D.ndata-1;
         while(l<=h){
       //  for(int d=0; d<D.ndata; d++) // For each interval, thread loops through all of data and  		
       	int mid1 = l + (h-l)/3;
       	int mid2 = h - (h-l)/3;
             if(D.data[mid1].value == r) {// If the data item is in this interval 
                 D2.data[rangecount[r-1]+rcount++] = D.data[mid1]; // Copy it to the appropriate place in D2.
                 break;}
       	//int mid2 = l + (2*(h-l))/3
             else if(D.data[mid2].value == r) {// If the data item is in this interval 
                 D2.data[rangecount[r-1]+rcount++] = D.data[mid2]; // Copy it to the appropriate place in D2.
                 break;}
