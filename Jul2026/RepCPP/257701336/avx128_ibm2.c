...
void print_content(float *a, int len)
{
    ...
    //for(int i = 0; i < len; i++) {
    //    tmp.f = a + i;
    //    fprintf(stdout, "%X  ", *tmp.i);
    //}

    //printf("print_content: %p\n", a);
...
//        loff -= (cfloat == 0) ? 4 : 0;
//    }
//    __m256i ymm_256 = vec_set8sw(e32[7], e32[6], e32[5], e32[4], e32[3], e32[2], e32[1], e32[0]);
//
...
//            e32[j] = -1;
//    }
//    __m256i ymm_256_2 = vec_set8sw(e32[7], e32[6], e32[5], e32[4], e32[3], e32[2], e32[1], e32[0]);
//
//    conv1.f = out;
...
//    int FP_BYTES = 4;
//    int FP_LEN = BYTE * FP_BYTES;
//    /* TODO Discard granularity: bytes */
//    int b;
//    if (ebits % BYTE) {
//        b = (ebits + BYTE - 1) / BYTE;
//        ebits = b * BYTE;
...
void pack_sse(float *in, int size, int ebits, float *out) 
{
    ...
    for (int i = 0; i < iters; i++) {
        ...
        //_mm_maskmoveu_si128(ymm2, ymm_128_2, conv1.c);
...
void unpack_sse(float *in, int size, int bits, float *out) 
{
    ...
    __m128i ymm_128 = vec_set16sb(e8[15], e8[14], e8[13], e8[12], e8[11], e8[10], e8[9], e8[8], \
                                   e8[7], e8[6], e8[5], e8[4], e8[3], e8[2], e8[1], e8[0]);

    unsigned int aux = ~0;
    aux <<= FP_LEN - bits;
    __m128i ymm_and = vec_set4sw(aux, aux, aux, aux);
    ...
    for (int i = 0; i < iters; i++) {
        ...
        __m128i ymm4 = vec_bitand1q(ymm2, ymm_and);

        vec_store1q((__m128i*) dstp.f, ymm4);
    ...
    for (int i = 0; i < rep; i++) {
        ...
        switch(mode) {
            ...
            case 2: pack_sse_omp(original, tot_float_in, ebits, packed);
					unpack_omp(packed, tot_float_in, ebits, restored);
                    break;
            //case 3: pack_sse(original, tot_float_in, ebits, packed);
			//		unpack_omp(packed, tot_float_in, ebits, restored);
            //        break;
            //case 4: pack_sse_omp(original, tot_float_in, ebits, packed);
			//		unpack_omp(packed, tot_float_in, ebits, restored);
            //        break;
