...
void tasyclCreateQueues(size_t count=TASYCL_NQUEUES_AUTO,
                        const sycl::property_list& propList = {});

void tasyclCreateQueues(const sycl::async_handler& asyncHandler,
                        size_t count=TASYCL_NQUEUES_AUTO,
                        const sycl::property_list& propList = {});
...
//!  Gets queue number 'i'.
sycl::queue&
tasyclGetQueue(short int queueId=TASYCL_QUEUE_ID_DEFAULT);

//! \brief Getting a queue
//!
//!  Gets a queue from the pool by passing a pointer to a previously declarated queue
//!  Gets queue number 'i'.
[[deprecated("use `sycl::queue tasyclGetQueue()` instead")]] void
...
//! \brief Apply function to every queue
