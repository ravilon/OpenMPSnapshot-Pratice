...
void QuickSortPara(int A[], int l, int r, int numThreads)
{
	//Lập số threads sẽ chạy song song
#pragma omp parallel num_threads(numThreads) 
	{//Định ra số threads sẽ chạy 

#pragma omp single nowait
		{//Gặp câu lệnh này sẽ chỉ cho một threads chạy câu lệnh QuickSortInter đây có thễ coi là thread chính 
		 //trong khi đó nowait thì sẽ không cần đồng bộ hay chờ đợi thread nào cả
		 //Nếu không có câu lệnh single thì tất cả số threads - numThreads sẽ chạy câu lệnh QuickSortInter
