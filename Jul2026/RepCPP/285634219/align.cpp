...
{
// set begin, end, query, and move it_
...
bool get()
{
//queue_ is a global variable
//Side effet is to gall operator() on success
...
{
Align_fetcher hits; // Definind new hits
Statistics stat;
while (hits.get()) { //set hits.begin, hits.end, hits.query, and move it_
...
for (size_t i = 0; i < mapper.n_targets(); ++i)
mapper.align_target(i, stat); //120 lines
Text_buffer *buf = 0;
buf = new Text_buffer;
