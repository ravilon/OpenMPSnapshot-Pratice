...
#pragma omp target teams distribute parallel for collapse(3)  map(tofrom: nrj)   map(to: ny, nz, ixstart, ixend, iystart, iyend, izstart, izend)  /*is_device_ptr( data_ ) */            reduction(+:nrj) 
...
#pragma omp target if( is_the_right_field )  map( tofrom                              : field [0:fSize] )
#pragma omp teams distribute parallel for collapse( 3 )
#elif defined( SMILEI_ACCELERATOR_GPU_OACC )
...
#pragma omp target if( is_the_right_field )  map( to                                  : field [0:fSize] )
#pragma omp teams distribute parallel for collapse( 3 )
#elif defined( SMILEI_ACCELERATOR_GPU_OACC )
const int subSize = sendFields_[iDim*2+iNeighbor]->size();
const int fSize = number_of_points_;
