    ...
    int ipart_ref )
{
    ...
    #else
        int np = iend-istart;
        #pragma acc parallel  present(Ex[istart:np],Ey[istart:np],Ez[istart:np], Bx[istart:np],By[istart:np],Bz[istart:np])  deviceptr(momentum_x,momentum_y,momentum_z,charge,weight,chi)  reduction(+:radiated_energy_loc)
    {
