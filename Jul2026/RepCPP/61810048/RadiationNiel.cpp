    ...
    int ipart_ref )
{
    ...
    // 1) Vectorized computation of gamma and the particle quantum parameter
            ...
            #pragma acc parallel create(random_numbers[0:nbparticles], diffusion[0:nbparticles])   present(Ex[istart:np],Ey[istart:np],Ez[istart:np], Bx[istart:np],By[istart:np],Bz[istart:np],gamma[istart:np],  radiation_tables,  radiation_tables.niel_,  radiation_tables.niel_.data_[0:niel_table_size] )  deviceptr(momentum_x,momentum_y,momentum_z,charge,weight,particle_chi)  private(temp,rad_energy,new_gamma, p, seed_curand ) reduction(+:radiated_energy_loc)
        {
