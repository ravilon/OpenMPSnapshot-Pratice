...
#pragma omp target teams distribute parallel for  map(tofrom: density)   is_device_ptr(weight_ptr)  reduction(+:density) 
...
#if defined( SMILEI_ACCELERATOR_GPU_OMP )
#pragma omp target  teams distribute parallel for  map(tofrom: charge)   is_device_ptr( charge_ptr, weight_ptr)  reduction(+:charge)  
#elif defined( SMILEI_ACCELERATOR_GPU_OACC )
...
#if defined( SMILEI_ACCELERATOR_GPU_OMP )
#pragma omp target teams distribute parallel for  map(tofrom: ener_tot)   is_device_ptr(weight_ptr,  momentum_x /* [istart:particle_number] */,              momentum_y /* [istart:particle_number] */,              momentum_z /* [istart:particle_number] */)              reduction(+:ener_tot) 
#elif defined(SMILEI_ACCELERATOR_GPU_OACC)
#pragma acc parallel deviceptr(weight_ptr,  momentum_x,                                            momentum_y,                                            momentum_z)
...
#pragma omp simd reduction(+:density)  reduction(+:charge)   reduction(+:ener_tot)
for( unsigned int iPart=0 ; iPart<nPart; iPart++ ) {
density  += weight_ptr[iPart];
charge   += weight_ptr[iPart] * charge_ptr[iPart];
const double gamma = std::sqrt(1 + momentum_x[iPart]*momentum_x[iPart] 
...
#pragma omp simd reduction(+:density)  reduction(+:ener_tot)
for( unsigned int iPart=0 ; iPart<nPart; iPart++ ) {
...
#if defined( SMILEI_ACCELERATOR_GPU_OMP )
#pragma omp target  teams distribute parallel for collpase(3)  map(tofrom: minval, maxval, i_min, i_max, j_min, j_max, k_min, k_max)   map(to: ny, nz, ixstart, ixend, iystart, iyend, izstart, izend) 
