...
#pragma omp target teams distribute parallel for collapse(2)  map(tofrom: nrj)   map(to: ny, idxlocalstart[0], idxlocalstart[1], iystart, iyend)  /* is_device_ptr( data_ )*/  reduction(+:nrj) 
...
#pragma omp target if( should_manipulate_gpu_memory )  map( tofrom                                        : field [field_first:field_last - field_first] )
#pragma omp teams distribute parallel for collapse( 2 )
#elif defined( SMILEI_ACCELERATOR_GPU_OACC )
...
#pragma omp target if( should_manipulate_gpu_memory )  map( to                                            : field [field_first:field_last - field_first] )
#pragma omp teams distribute parallel for collapse( 2 )
#elif defined( SMILEI_ACCELERATOR_GPU_OACC )
const int subSize = sendFields_[iDim*2+iNeighbor]->size();
const int fSize = number_of_points_;
