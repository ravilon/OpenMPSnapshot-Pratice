 ...
 * @date  3/20/21 - 3/30/21, 4/25/22 - 5/15/22
 */
...
// Convert a float to a formatted string
...
Simulation::Simulation() {
    ...
    // Determine the maximum threads available
    settings->maxThreads = omp_get_max_threads();
    settings->realtime = false;
    // If there are more than one, set it to two for start
    settings->numThreads = settings->maxThreads > 2 ? settings->maxThreads : 1;
    // Initialize the graphics engine (only in windowed mode)
    graphics = new Graphics(1024 + 256 + 16, 1024 + 48);
    graphics->centerFull(1024 - 32 - 256);
}

// Begin custom simulation
Simulation::Simulation(Settings *s) {
    // Assign the provided settings
    settings = s;
    // Calculate the max threads available
    settings->maxThreads = omp_get_max_threads();
    // If the user defined max threads, set it
    if (settings->numThreads == -1) {
        settings->numThreads = omp_get_max_threads();
    }
    // Leave graphics null in headless mode
    graphics = nullptr;
...
void Simulation::simulate() {
    ...
    // Calculate and add the force between every unique pair of bodies
#pragma omp parallel for default(none) shared(bodies, settings) num_threads(settings->numThreads) if(settings->useParallel)
    ...
    // Update the positions of all the points with the recently calculated new forces.
