...
{
ImageClass<float> input_image(argv[1]);  //path to image
ImageClass<float> result(input_image.width, input_image.height);
printf("\nImage size: %d x %d\n\n", input_image.width, input_image.height);

const double t0 = omp_get_wtime();
Convolve(input_image, result); //3x3 edge detection kernel
const double t1 = omp_get_wtime();
printf("\nPRINTING STATISTICS\n\n");
const double ts   = t1-t0; // time in seconds
const double gbps = double(sizeof(float )*input_image.width*input_image.height*2)*1e-9/ts; // bandwidth in GB/s
const double fpps = double(input_image.width*input_image.height*2*9)*1e-9/ts; // performance in GFLOP/s
