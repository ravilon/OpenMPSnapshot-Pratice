...
str.compare(str.size() - suffixLen, suffixLen, suffix, suffixLen) == 0;
}
...
if (!fs_path.is_absolute() && !fs_path.is_relative()) return false;
...
if (path.empty() || valid_extensions.empty()) return false;
