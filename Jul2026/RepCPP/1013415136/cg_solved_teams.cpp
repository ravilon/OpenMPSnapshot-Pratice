...
#pragma omp target data map(alloc:r[0:n]) map(values[0:values_size]) map(col_indices[0:values_size]) map(row_start[0:n+1]) map(b[0:n]) map(x[0:n]) map(alloc:p[0:n]) map(alloc:Ap[0:n]) map(alloc:Ax[0:n])
{
