...
/* tofrom: default*/
#pragma omp target data map(solid[0 : total_size])                                                                                                      \
    map(tofrom : rho[0 : total_size]) map(tofrom : rhou[0 : total_size]) map(tofrom : rhov[0 : total_size]) map(tofrom : E[0 : total_size])             \
    ...
    map(alloc : kinetics_printed[0 : 40])
    {
