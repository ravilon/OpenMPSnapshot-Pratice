...
/*
 * NOTE
 * In the first section, the code acquires locka and then attempts to acquire lockb. 
 * In the second section, the code acquires lockb and then tries to acquire locka. 
 * If both sections run concurrently, each will hold one lock and wait for the other, causing a deadlock.
 */
