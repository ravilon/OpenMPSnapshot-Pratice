...
#if defined(_OPENMP)
#pragma omp declare reduction(sm : std::complex<T> : omp_out =                  ConstSum(omp_out, omp_in))                    initializer(omp_priv = std::complex<T>{0, 0})
...
#pragma omp parallel for num_threads(nthreads) default(none)                    shared(v1, v2, data_size) reduction(sm : result)
#endif
...
#if defined(_OPENMP)
#pragma omp parallel for default(none) private(row, col) firstprivate(m, n)     shared(v_out, mat, v_in)
#endif
            for (row = 0; row < m; row++) {
                for (col = 0; col < n; col++) {
...
inline void omp_matrixMatProd(const std::complex<T> *m_left,
    ...
    case Trans::Transpose:
#if defined(_OPENMP)
#pragma omp parallel for default(none) shared(m_left, m_right, m_out)           firstprivate(m, n, k)
#endif
