...
#pragma omp parallel for reduction(+ : expected_value) default(none)            shared(arr) firstprivate(num_qubits, wire_parity_inv, wire_parity,          rev_wire_shift, two2N, core_function)
...
#pragma omp parallel for reduction(+ : expected_value) default(none)            shared(arr) firstprivate(num_qubits, wire_parity_inv, wire_parity,          rev_wire_shift, matrix, two2N)
for (std::size_t k = 0; k < two2N; k++) {
const std::size_t i0 =
((k << 1U) & wire_parity_inv) | (wire_parity & k);
const std::size_t i1 = i0 | rev_wire_shift;
...
#pragma omp parallel for reduction(+ : expected_value) default(none)            shared(arr) firstprivate(num_qubits, rev_wire0, rev_wire1,                  rev_wire0_shift, rev_wire1_shift, parity_low,  parity_high, parity_middle, matrix, two2N)
for (std::size_t k = 0; k < two2N; k++) {
...
#pragma omp parallel for reduction(+ : expected_value) default(none)            shared(arr)                                                                 firstprivate(num_qubits, parity, rev_wire_shifts, matrix, two2N)
for (std::size_t k = 0; k < two2N; k++) {
...
const std::size_t two2N = PUtil::exp2(num_qubits - wires.size());
#pragma omp parallel for reduction(+ : expected_value) default(none)            shared(arr, matrix)                                                         firstprivate(dim, wires, num_qubits, parity, rev_wire_shifts, two2N)
for (std::size_t k = 0; k < two2N; ++k) {
