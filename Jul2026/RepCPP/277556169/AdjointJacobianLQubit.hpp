  ...
  private:
    ...
    inline void applyObservables(
        ...
        if (num_observables > 1) {
            ...
            #if defined(_OPENMP)
                #pragma omp parallel default(none)                                  shared(states, reference_state, observables, ex, num_observables)
            {
                #pragma omp for
            #endif
    ...
    inline void applyOperationsAdj(std::vector<StateVectorT> &states,
        ...
        #if defined(_OPENMP)
            #pragma omp parallel default(none)                                  shared(states, operations, op_idx, ex, num_states)
        {
            #pragma omp for
        #endif
    ...
    void adjointJacobian(std::span<PrecisionT> jac,
        ...
        for (int op_idx = static_cast<int>(ops_name.size() - 1); op_idx >= 0;
            ...
            if (ops.hasParams(op_idx)) {
                ...
                #if defined(_OPENMP)
                #pragma omp parallel for default(none)                          shared(H_lambda, jac, mu, scalingFactor, mat_row_idx,       num_observables)
                #endif
                    // clang-format on
