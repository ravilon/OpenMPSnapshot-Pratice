...
#ifndef LBT__MATH__CEIL_TEST
#define LBT__MATH__CEIL_TEST
#pragma once
  ...
  namespace test {
    ...
    TYPED_TEST(CeilTest, zeroIsZero) {
      ...
      if constexpr (std::is_same_v<TypeParam,float>) {
        EXPECT_FLOAT_EQ(lbt::math::ceil(zero), zero);
      } else if constexpr (std::is_same_v<TypeParam,double>) {
        EXPECT_DOUBLE_EQ(lbt::math::ceil(zero), zero);
      } else {
    ...
    TYPED_TEST(CeilTest, positiveNumbersAreCorrect) {
      ...
      for (auto const& [n, solution]: tests) {
        if constexpr (std::is_same_v<TypeParam,float>) {
          EXPECT_FLOAT_EQ(lbt::math::ceil(n), solution);
        } else if constexpr (std::is_same_v<TypeParam,double>) {
          EXPECT_DOUBLE_EQ(lbt::math::ceil(n), solution);
        } else {
          GTEST_SKIP() << "Test not implemented for given data type!";
        }
      }
    ...
    TYPED_TEST(CeilTest, positiveNumbersAreEqualToStdSqrt) {
      ...
      for (auto const& n: positive_numbers) {
        if constexpr (std::is_same_v<TypeParam,float>) {
          EXPECT_FLOAT_EQ(lbt::math::ceil(n), std::ceil(n));
        } else if constexpr (std::is_same_v<TypeParam,double>) {
          EXPECT_DOUBLE_EQ(lbt::math::ceil(n), std::ceil(n));
        } else {
          GTEST_SKIP() << "Test not implemented for given data type!";
        }
      }
