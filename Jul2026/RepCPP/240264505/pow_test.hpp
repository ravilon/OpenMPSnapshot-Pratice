...
#ifndef LBT__MATH__POW_TEST
#define LBT__MATH__POW_TEST
#pragma once
...
#include "lbt/math/detail/pow.hpp"
...
EXPECT_TRUE(lbt::math::isNan(lbt::math::pow(zero, nan)));
}
...
EXPECT_TRUE(lbt::math::isAlmostEqualEpsAbs(lbt::math::pow(b, zero), unity));
}
}
...
EXPECT_TRUE(lbt::math::isPosInf(lbt::math::pow(zero, inf)));
}

TYPED_TEST(PowTest, zeroToThePowerOfPositiveInfinityIsZero) {
constexpr auto zero {static_cast<TypeParam>(0.0)};
constexpr auto inf {std::numeric_limits<TypeParam>::infinity()};
EXPECT_TRUE(lbt::math::isAlmostEqualEpsAbs(lbt::math::pow(zero, inf), zero));
}
...
EXPECT_TRUE(lbt::math::isNan(lbt::math::pow(base, exponent)));
}
}
...
EXPECT_TRUE(lbt::math::isPosInf(lbt::math::pow(inf, e)));
}
}
...
for (auto const& e: exponents) {
EXPECT_TRUE(lbt::math::isNegInf(lbt::math::pow(inf, e)));
}
}
...
for (auto const& e: exponents) {
EXPECT_TRUE(lbt::math::isAlmostEqualEpsAbs(lbt::math::pow(inf, e), zero));
}
}
...
constexpr auto neg_inf {-std::numeric_limits<TypeParam>::infinity()};
constexpr auto pos_inf {std::numeric_limits<TypeParam>::infinity()};
EXPECT_TRUE(lbt::math::isPosInf(lbt::math::pow(neg_inf, pos_inf)));
}
...
for (auto const& [base, exponent]: base_exponent) {
EXPECT_TRUE(lbt::math::isAlmostEqualEpsAbs(lbt::math::pow(base, exponent), zero));
}
}
...
for (auto const& [base, exponent]: base_exponent) {
EXPECT_TRUE(lbt::math::isPosInf(lbt::math::pow(base, exponent)));
}
}
...
for (auto const& e: exponents) {
EXPECT_TRUE(lbt::math::isAlmostEqualEpsAbs(lbt::math::pow(zero, e), zero));
}
}
...
std::vector<TypeParam> const exponents = { -0.9, -4.0, -6.3};
for (auto const& e: exponents) {
EXPECT_TRUE(lbt::math::isPosInf(lbt::math::pow(zero, e)));
}
}
...
EXPECT_FLOAT_EQ(lbt::math::pow(base, exponent), solution);
} else if constexpr (std::is_same_v<TypeParam,double>) {
EXPECT_DOUBLE_EQ(lbt::math::pow(base, exponent), solution);
} else {
...
for (auto const& [base, exponent]: base_exponent) {
if constexpr (std::is_same_v<TypeParam,float>) {
EXPECT_FLOAT_EQ(lbt::math::pow(base, exponent), std::pow(base, exponent));
} else if constexpr (std::is_same_v<TypeParam,double>) {
EXPECT_DOUBLE_EQ(lbt::math::pow(base, exponent), std::pow(base, exponent));
} else {
GTEST_SKIP() << "Test not implemented for given data type!";
}
}
