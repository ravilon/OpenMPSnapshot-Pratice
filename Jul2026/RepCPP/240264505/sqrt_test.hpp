...
#ifndef LBT__MATH__SQRT_TEST
#define LBT__MATH__SQRT_TEST
#pragma once
...
#include "lbt/math/detail/sqrt.hpp"
...
namespace lbt {
  namespace test {
    ...
    TYPED_TEST(SqrtTest, positiveInfinityStaysPositiveInfinity) {
      ...
      EXPECT_TRUE(std::isinf(lbt::math::sqrt(inf)));
    }

    TYPED_TEST(SqrtTest, positiveZeroStaysPositiveZero) {
      constexpr TypeParam positive_zero {+0.0};
      if constexpr (std::is_same_v<TypeParam,float>) {
        EXPECT_FLOAT_EQ(lbt::math::sqrt(positive_zero), positive_zero);
      } else if constexpr (std::is_same_v<TypeParam,double>) {
        EXPECT_DOUBLE_EQ(lbt::math::sqrt(positive_zero), positive_zero);
      } else {
    ...
    TYPED_TEST(SqrtTest, negativeZeroStaysNegativeZero) {
      ...
      if constexpr (std::is_same_v<TypeParam,float>) {
        EXPECT_FLOAT_EQ(lbt::math::sqrt(negative_zero), negative_zero);
      } else if constexpr (std::is_same_v<TypeParam,double>) {
        EXPECT_DOUBLE_EQ(lbt::math::sqrt(negative_zero), negative_zero);
      } else {
        GTEST_SKIP() << "Test not implemented for given data type!";
      }
    }
    ...
    TYPED_TEST(SqrtTest, signalingNanIsNan) {
      ...
      EXPECT_TRUE(std::isnan(lbt::math::sqrt(nan)));
    }
    ...
    TYPED_TEST(SqrtTest, positiveNumbersCorrectSqrt) {
      ...
      for (auto const& [n, solution]: tests) {
        if constexpr (std::is_same_v<TypeParam,float>) {
          EXPECT_FLOAT_EQ(lbt::math::sqrt(n), solution);
        } else if constexpr (std::is_same_v<TypeParam,double>) {
          EXPECT_DOUBLE_EQ(lbt::math::sqrt(n), solution);
        } else {
          GTEST_SKIP() << "Test not implemented for given data type!";
        }
      }
    ...
    TYPED_TEST(SqrtTest, positiveNumbersEqualToStdSqrt) {
      ...
      for (auto const& n: tests) {
        if constexpr (std::is_same_v<TypeParam,float>) {
          EXPECT_FLOAT_EQ(lbt::math::sqrt(n), std::sqrt(n));
        } else if constexpr (std::is_same_v<TypeParam,double>) {
          EXPECT_DOUBLE_EQ(lbt::math::sqrt(n), std::sqrt(n));
        } else {
          GTEST_SKIP() << "Test not implemented for given data type!";
        }
      }
    ...
    TYPED_TEST(SqrtTest, randomNumberSquaredCorrectSqrt) {
      ...
      if constexpr (std::is_same_v<TypeParam,float>) {
        EXPECT_FLOAT_EQ(lbt::math::sqrt(random_number_squared), random_number);
      } else if constexpr (std::is_same_v<TypeParam,double>) {
        EXPECT_DOUBLE_EQ(lbt::math::sqrt(random_number_squared), random_number);
      } else {
        GTEST_SKIP() << "Test not implemented for given data type!";
      }
    }
    ...
    TYPED_TEST(SqrtTest, smallestPositiveNumberLargerSqrt) {
      ...
      EXPECT_GT(lbt::math::sqrt(smallest_positive_number), smallest_positive_number);
    }

    TYPED_TEST(SqrtTest, smallestNegativeNumberIsNan) {
      auto const smallest_negative_number {-std::numeric_limits<TypeParam>::min()};
      EXPECT_TRUE(std::isnan(lbt::math::sqrt(smallest_negative_number)));
    }

  }
}

#endif // LBT__MATH__SQRT_TEST
