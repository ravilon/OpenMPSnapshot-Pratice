...
#ifndef LBT__MATH__ABS_TEST
#define LBT__MATH__ABS_TEST
#pragma once
  ...
  namespace test {
    ...
    TYPED_TEST(AbsTest, negativeZeroIsPositiveZero) {
      if constexpr (std::is_floating_point_v<TypeParam>) {
        ...
        if constexpr (std::is_same_v<TypeParam,float>) {
          EXPECT_FLOAT_EQ(lbt::math::abs(negative_zero), positive_zero);
        } else if constexpr (std::is_same_v<TypeParam,double>) {
          EXPECT_DOUBLE_EQ(lbt::math::abs(negative_zero), positive_zero);
        } else {
      ...
      if constexpr (std::is_signed_v<TypeParam>) {
        ...
        for (auto const& [n, p]: tests) {
          if constexpr (std::is_integral_v<TypeParam>) {
            EXPECT_EQ(lbt::math::abs(n), p);
          } else if constexpr (std::is_same_v<TypeParam,float>) {
            EXPECT_FLOAT_EQ(lbt::math::abs(n), p);
            EXPECT_FLOAT_EQ(lbt::math::abs(n), std::abs(n));
          } else if constexpr (std::is_same_v<TypeParam,double>) {
            EXPECT_DOUBLE_EQ(lbt::math::abs(n), p);
            EXPECT_DOUBLE_EQ(lbt::math::abs(n), std::abs(n));
          } else {
            GTEST_SKIP() << "Test not implemented for given data type!";
          }
        }
    ...
    TYPED_TEST(AbsTest, positiveNumberStaysPositiveNumber) {
      ...
      for (auto const& n: positive_numbers) {
        if constexpr (std::is_integral_v<TypeParam>) {
          EXPECT_EQ(lbt::math::abs(n), n);
        } else if constexpr (std::is_same_v<TypeParam,float>) {
          EXPECT_FLOAT_EQ(lbt::math::abs(n), n);
        } else if constexpr (std::is_same_v<TypeParam,double>) {
          EXPECT_DOUBLE_EQ(lbt::math::abs(n), n);
        } else {
          GTEST_SKIP() << "Test not implemented for given data type!";
        }
      }
    ...
    TYPED_TEST(AbsTest, negativeNumberIsEqualToStdAbs) {
      if constexpr (std::is_signed_v<TypeParam>) {
        ...
        for (auto const& n: negative_numbers) {
          if constexpr (std::is_integral_v<TypeParam>) {
            EXPECT_EQ(lbt::math::abs(n), std::abs(n));
          } else if constexpr (std::is_same_v<TypeParam,float>) {
            EXPECT_FLOAT_EQ(lbt::math::abs(n), std::abs(n));
          } else if constexpr (std::is_same_v<TypeParam,double>) {
            EXPECT_DOUBLE_EQ(lbt::math::abs(n), std::abs(n));
