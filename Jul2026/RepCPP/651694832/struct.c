...
s.i = 20;
s.j = 30;
#pragma omp target data map(tofrom : s) map(ompx_hold, tofrom : s.i)            map(tofrom : s.j)
{
...
s.i = 20;
s.j = 30;
#pragma omp target data map(tofrom : s) map(tofrom : s.i)                       map(ompx_hold, tofrom : s.j)
{
// CHECK-NEXT: presence of s, s.i, s.j: 1, 1, 1
CHECK_PRESENCE(s, s.i, s.j);
#pragma omp target map(tofrom : s)
...
s.i = 20;
s.j = 30;
#pragma omp target data map(ompx_hold, tofrom : s) map(tofrom : s.i)            map(tofrom : s.j)
{
// CHECK-NEXT: presence of s, s.i, s.j: 1, 1, 1
CHECK_PRESENCE(s, s.i, s.j);
#pragma omp target map(tofrom : s)
