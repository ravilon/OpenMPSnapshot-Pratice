...
#pragma omp target teams distribute parallel for device(src_device)             is_device_ptr(src_ptr)
for (int i = 0; i < N; ++i) {
src_ptr[i] = magic_num;
}
...
#pragma omp target teams distribute parallel for device(dst_device)             map(from : buffer[0 : N]) is_device_ptr(dst_ptr)
for (int i = 0; i < N; ++i) {
